	...
	if ($posts) {
		...
		foreach ($posts as $post)
		{
			...
			$html .= '<div class="__wps__threads_div">';
				...
				$html .= '<div>';
				if ($post->t2_topic_parent == 0) {
					$html .= __('Reply by', WPS_TEXT_DOMAIN).' '.$post->display_name.', '.__wps__time_ago($post->topic_date).' '.__('in', WPS_TEXT_DOMAIN).' ';
				} else {
					$html .= __('Comment by', WPS_TEXT_DOMAIN).' '.$post->display_name.', '.__wps__time_ago($post->topic_date).' '.__('in', WPS_TEXT_DOMAIN).' ';
				}
