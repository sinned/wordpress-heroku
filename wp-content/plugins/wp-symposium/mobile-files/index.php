...
if (isset($_POST['post_comment']) && $_POST['post_comment'] != '' && $current_user->ID > 0) {
	...
	if ($current_user->ID != $wps_user->id) {

		if (__wps__get_meta($wps_user->id, 'notify_new_wall') == 'on') {
			// Filter to allow further actions to take place
			apply_filters ('__wps__wall_newpost_filter', $wps_user->id, $current_user->ID, $current_user->display_name );

			$sql = "SELECT u.user_email FROM ".$wpdb->base_prefix."users u WHERE u.ID = %d";
			$recipient = $wpdb->get_row($wpdb->prepare($sql, $wps_user->id));	
			...
			if ($recipient) {
				...
				$body .= "<p><a href='".__wps__get_url('profile')."?uid=".$wps_user->id."&post=".$new_id."'>".__('Go to the post', WPS_TEXT_DOMAIN)."...</a></p>";
...
if ( !is_user_logged_in() ) {
	...
	$comments = $wps_user->get_activity($uid1='', $version='friends_activity', 0, 100);
	...
	if ($comments) {	
		...
		foreach ($comments as $comment) {
			...
			if  ( ($comment->type != 'poke') || ($comment->type == 'poke' && ($comment->author_uid == $uid2 || $comment->subject_uid == $uid2 )) ) {	
				...
				echo "<div class='__wps__profile_activity_div' id='post_".$comment->cid."'>";
					...
					$text = str_replace(__('Started a new forum topic:', WPS_TEXT_DOMAIN), __('Started ', WPS_TEXT_DOMAIN), $text);

					// Change any forum links to mobile link
					// Skip this as mobile will soon be depricated and replaced with responsive WPS design
					if ( false && get_option(WPS_OPTIONS_PREFIX.'_permalink_structure') && strpos($text, $forum_url) !== false) {
					...
					echo __wps__make_url(stripslashes($text));
					echo '</div>';
					
					// How many replies?
					echo '<br />';
					$replies = $wps_user->get_replies($comment->cid);
					if ($replies) {
						...
						echo '<input type="submit" onclick="location.href=\'activity_comments.php?'.$a.'&tid='.$comment->cid.'\'" class="submit nowidth small orange floatleft" value="'.__('Comment', WPS_TEXT_DOMAIN).'" />';
					}
